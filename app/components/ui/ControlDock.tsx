  ...
  return (
    ...
    >
      <div 
        ...
        style={{
          background: `linear-gradient(135deg, ${minecraftTheme.colors.beige.base}F8 0%, ${minecraftTheme.colors.beige.light}F5 100%)`,
          border: `3px solid ${minecraftTheme.colors.terracotta.base}`,
          boxShadow: `${minecraftTheme.minecraft.shadowRaised}, 0 8px 24px rgba(0,0,0,0.2)`,
...
function DockButton({ icon, label, isActive, onClick, color, activeColor }: DockButtonProps) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.92 }}
      onClick={onClick}
      className="relative flex flex-col items-center justify-center w-16 h-16 rounded-xl transition-all group overflow-hidden"
      style={{
        background: isActive 
    ...
    >
      ...
      <span 
        className="text-2xl mb-1 relative z-10 transition-all"
        style={{ 
