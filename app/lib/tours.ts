...
import { getMuseumGuideFact } from './museumGuideFacts'
...
export function createMuseumTour(museum: MuseumTourSource): Tour {
  ...
  const guideFact = getMuseumGuideFact(museum.name)
    ...
    cards: [
      {
        ...
        display: `${description} ${guideFact.signature}`,
        voice:
          `You are at ${museum.name}. Here is the quick guide version: ${description} ${guideFact.signature}`,
      },
      {
        kind: 'history',
        title: 'Why it matters',
        display: guideFact.whyItMatters,
        voice:
          `${shortName} matters because ${guideFact.whyItMatters}`,
      },
      {
        kind: 'surprise',
        title: 'Hidden detail',
        display: guideFact.hiddenDetail,
        voice:
          `Here is the detail I would point out if we were walking together: ${guideFact.hiddenDetail}`,
      },
      {
        kind: 'tip',
        title: 'How to visit it well',
        display: `${guideFact.visitMission} Address: ${address}. Nearby idea: ${guideFact.nearbyPairing}`,
        voice:
          `If you were here with a guide, I would give you one mission: ${guideFact.visitMission} The address is ${address}. Nearby idea: ${guideFact.nearbyPairing}`,
      },
    ],
  }
}
